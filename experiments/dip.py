...
from subspace_dip.dip import SubspaceDeepImagePrior, SamplerTrainer
...
def coordinator(cfg : DictConfig) -> None:
    ...
    dataset = get_standard_dataset(
            cfg, ray_trafo, use_fixed_seeds_starting_from=cfg.seed,
            device=device, use_adp_dataset=True)
    ...
    for i, data_sample in enumerate(islice(DataLoader(dataset), cfg.num_images)):
        ...
        recon = reconstructor.reconstruct(
                subspace = sample_trainer.subspace, 
                mean = sample_trainer.mean, 
