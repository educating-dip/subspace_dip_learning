...
from subspace_dip.utils import get_updated_func_params



class SubspaceDeepImagePrior():
    """
    ...
    def reconstruct(self,
        ...
        coeffs = nn.Parameter(
            torch.randn(
                subspace.shape[-1], requires_grad=True, device=self.device)
            )
        ...
        func_model_with_input, _ = ftch.make_functional(self.nn_model)
        ...
        with tqdm(range(optim_kwargs['iterations']), desc='DIP', disable=not show_pbar,
            ...
            for i in pbar:
                ...
                func_params = get_updated_func_params(self.nn_model, coeffs, subspace, mean)
                output = func_model_with_input(func_params, self.net_input)
